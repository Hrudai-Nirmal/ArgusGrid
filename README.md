...
Billing is a dedicated sidebar section for beta pricing, prepaid credits, project usage graphs, and the project operations policy. The first beta scheme is intentionally conservative: Free Sandbox is $0 / INR 0, Solo Beta is $59 / INR 4,999, Agency Beta is $179 / INR 14,999, and Enterprise Pilot is $799 / INR 64,999. Prepaid credit packs are 500 credits for $19 / INR 1,599, 2,000 credits for $69 / INR 5,999, and 10,000 credits for $249 / INR 20,999. Paddle Billing is the primary SaaS checkout path for paid plans and credit packs once `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN` and the matching Paddle `pri_...` price IDs are configured. Billing also keeps a `Test INR 1 payment` Razorpay fallback button for India gateway comparison while Paddle is evaluated. Paid-plan buttons are labeled `Upgrade to Solo Beta`, `Upgrade to Agency Beta`, and `Upgrade to Enterprise Pilot`; credit-pack buttons are labeled `Buy 500 credits`, `Buy 2,000 credits`, and `Buy 10,000 credits`.
...
Billing includes Paddle Billing overlay checkout for paid plan and credit-pack test purchases. The frontend loads `https://cdn.paddle.com/paddle/v2/paddle.js`, initializes Paddle.js with the browser-safe `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`, sets sandbox mode when `NEXT_PUBLIC_PADDLE_ENVIRONMENT=sandbox`, then opens checkout with the configured `NEXT_PUBLIC_PADDLE_PRICE_*` price ID. Paddle client-side tokens and price IDs are safe to expose; do not add a server Paddle API key to browser code. The current Paddle path opens and completes checkout only. Plan activation, credit ledger entries, invoices, customer portal, and webhook-backed fulfilment are the next billing milestone before taking real customer payments.

Razorpay Standard Web Checkout remains as a fallback test path only. The frontend loads `https://checkout.razorpay.com/v1/checkout.js`, calls `POST /api/create-order`, opens the Razorpay modal with the public `key_id` returned by that authenticated order response, then sends `razorpay_payment_id`, `razorpay_order_id`, and `razorpay_signature` to `POST /api/verify-payment`. The server verifies the signature using `RAZORPAY_KEY_SECRET`; the key secret must never be exposed to the browser. `NEXT_PUBLIC_RAZORPAY_KEY_ID` is kept as an optional local fallback, but production checkout works from the server-side `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET`. The INR 1 checkout pre-fills safe customer fields and leaves payment-method selection to Razorpay because UPI QR and specific netbanking banks may be unavailable in test mode. Contact prefill uses a non-repeating test mobile number because Razorpay can reject obvious dummy values before payment-method entry.
...
Enterprise pilot builds apply baseline browser security headers through `next.config.ts`: `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options`, and HSTS. The permissions policy keeps sensitive defaults denied, with a narrow Razorpay allowance for checkout/payment risk checks. Public report routes remain token-scoped, revoked/expired reports return 404, and report pages/assets are configured as no-store. Production smoke verifies the main app shell and `/api/health` include the expected browser hardening headers. Owner/admin-only routes cover telemetry-token management, webhook/Slack destinations, report shares/presets, CSV exports, manual polling, notification-job retry/cancel, and production observability.
