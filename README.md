...
Billing is a dedicated sidebar section for beta pricing, prepaid credits, project usage graphs, and the project operations policy. The first beta scheme is intentionally conservative: Free Sandbox is $0 / INR 0, Solo Beta is $59 / INR 4,999, Agency Beta is $179 / INR 14,999, and Enterprise Pilot is $799 / INR 64,999. Prepaid credit packs are 500 credits for $19 / INR 1,599, 2,000 credits for $69 / INR 5,999, and 10,000 credits for $249 / INR 20,999. Paddle Billing is the sole checkout path for paid plans and credit packs once `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`, server-side Paddle API credentials, the signed webhook secret, and the matching Paddle `pri_...` price IDs are configured. Paid-plan buttons are labeled `Upgrade to Solo Beta`, `Upgrade to Agency Beta`, and `Upgrade to Enterprise Pilot`; credit-pack buttons are labeled `Buy 500 credits`, `Buy 2,000 credits`, and `Buy 10,000 credits`.
...
Billing includes Paddle Billing overlay checkout for paid plan and credit-pack purchases. The frontend loads `https://cdn.paddle.com/paddle/v2/paddle.js`, initializes Paddle.js with the browser-safe `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`, sets sandbox mode when `NEXT_PUBLIC_PADDLE_ENVIRONMENT=sandbox`, then opens checkout with the configured `NEXT_PUBLIC_PADDLE_PRICE_*` price ID. Paddle client-side tokens and price IDs are safe to expose; do not add a server Paddle API key to browser code. Server-side fulfillment is webhook-backed through `POST /api/paddle/webhook`, which verifies the raw request with `PADDLE_NOTIFICATION_WEBHOOK_SECRET` before mirroring safe customer, subscription, and transaction state into Postgres. Billing shows that mirrored subscription state, recent transaction history, and a `Manage subscription` action that mints a short-lived Paddle customer portal URL for owners/admins.

Billing entitlements are derived only from verified server-side Paddle evidence, never from a browser checkout event. Active, trialing, and past-due subscriptions choose the plan; if a paid plan transaction arrives before the subscription webhook, Meridian grants 24 hours of provisional access so a successful payer does not get blocked while Paddle notifications catch up. Plan limits and prepaid credits are enforced for workflow-run ingestion, persisted REST metric samples, and report-share creation. Denied writes return `402` with a safe reason, plan, limit, usage, and remaining-credit summary, and they also create secret-safe Billing log evidence for support review. Notification jobs are counted in Billing entitlement usage now; hard blocking notification delivery remains deferred so critical alert delivery is not accidentally suppressed during early beta.
...
5. Redeploy, open Billing, complete a sandbox checkout, then confirm the subscription panel and Billing history update after the signed webhook arrives.
