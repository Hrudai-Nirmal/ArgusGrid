/**
 * WebGL rim-light button used by the public pricing cards.
 */

"use client"

import { type CSSProperties, type MouseEventHandler, type ReactNode, type Ref, useEffect, useRef } from "react"
...
/**
 * Renders a specular button or link without exposing checkout/payment credentials.
 */
export default function SpecularButton({
  ...
  if (href) {
    return (
      <a ref={elementRef as Ref<HTMLAnchorElement>} href={disabled ? undefined : href} aria-disabled={disabled ? "true" : undefined} className={classes} style={style}>
  ...
  return (
    <button ref={elementRef as Ref<HTMLButtonElement>} type={type} disabled={disabled} onClick={onClick} className={classes} style={style}>
      <span ref={fxRef} className="specular-button__fx" aria-hidden="true" />
      <span className="specular-button__label">{children}</span>
    </button>
  )
