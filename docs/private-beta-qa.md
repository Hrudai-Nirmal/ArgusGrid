...
- Confirm the hero shows the cursor-reactive DotField background behind the headline and glass panels.
- Confirm the homepage shows the hero, Automation Map visual, integrations, client proof/reporting, Reliability section, pricing preview, and sign-in/register section.
- Confirm the homepage header is a centered fixed rounded bar instead of a full-width header.
- Confirm private-beta pricing CTAs use the specular button treatment and still navigate to sign-in/register.
...
- Click `Copy support packet` and confirm the copied `Billing support packet` includes plan, entitlement source, sync state, usage, credits, warnings, and recent safe denial/credit events without exposing provider secrets, raw payment payloads, webhook URLs, raw tokens, or environment values.
