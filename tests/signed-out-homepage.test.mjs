...
test("homepage header is a centered fixed rounded bar", async () => {
  ...
  assert.match(landing, /backgroundOpacity=\{0\.1\}/)
  assert.match(landing, /saturation=\{2\.1\}/)
  ...
  assert.match(glassSurfaceCss, /--glass-rim/)
  assert.match(glassSurfaceCss, /border: 1px solid var\(--glass-rim\)/)
  assert.match(glassSurfaceCss, /background:.*var\(--glass-frost/)
  assert.match(glassSurfaceCss, /\.glass-surface::before/)
  assert.match(glassSurfaceCss, /\.glass-surface::after/)
  assert.match(glassSurfaceCss, /backdrop-filter: blur\(22px\) saturate\(1\.65\) contrast\(1\.08\)/)
  assert.match(glassSurfaceCss, /drop-shadow\(1px 0 0 rgb\(56 189 248 \/ 24%\)\)/)
  assert.match(glassSurfaceCss, /box-shadow:.*0 1px 2px rgb\(255 255 255 \/ 62%\) inset/s)
...
test("homepage uses subtle BorderGlow for content cards and full BorderGlow for login and billing", async () => {
  ...
  assert.match(landing, /<GlowFrame key=\{title\} className="h-full" radius=\{20\} fillOpacity=\{subtleGlowFillOpacity\}>/)
  assert.match(landing, /<GlowFrame radius=\{32\} fillOpacity=\{subtleGlowFillOpacity\}>\s*<InteractiveAutomationMap \/>/)
  assert.match(landing, /<GlowFrame key=\{card\.title\} className="h-full" fillOpacity=\{subtleGlowFillOpacity\}>/)
  assert.match(landing, /<GlowFrame radius=\{32\} fillOpacity=\{subtleGlowFillOpacity\}>\s*<IntegrationBeamDemo \/>/)
  assert.match(landing, /<GlowFrame key=\{item\.title\} className="h-full" radius=\{20\} fillOpacity=\{subtleGlowFillOpacity\}>/)
  assert.match(landing, /<GlowFrame radius=\{32\} fillOpacity=\{subtleGlowFillOpacity\}>\s*<div className=\{cn\("rounded-\[2rem\]/)
